...
public:
	...
	virtual CBoundingBox calcBounds(void) const override
	{
		CBoundingBox res;
		// --- PUT YOUR CODE HERE ---
		Vec3f maximum_point(m_center[0] + m_radius, m_center[1] + m_radius, m_center[2] + m_radius);
		Vec3f minimum_point(m_center[0] - m_radius, m_center[1] - m_radius, m_center[2] - m_radius);
		res.extend(maximum_point);
		res.extend(minimum_point);
		return res;
	}
